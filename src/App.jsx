...
function getPrevMonths(monthlyData) {
  return Object.keys(monthlyData||{}).sort().reverse().slice(0, 6);
}
                  ...
                  <div key={r.id} onClick={() => { setSelectedRepLogin(r); setError(""); }} style={{ background:sel?`${track.color}18`:"#ffffff06", border:`1px solid ${sel?track.color+"50":"#ffffff10"}`, borderRadius:10, padding:"12px 14px", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    <div>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        {r.photo && <img src={r.photo} alt="" style={{ width:36, height:36, borderRadius:"50%", objectFit:"cover", border:`2px solid ${sel?track.color:"#ffffff30"}` }} />}
                        <div>
...
function AppointmentReminderBanner() {
  ...
  return (
    <div style={{
      background: "linear-gradient(135deg,#f59e0b18,#f43f5e0f)",
      ...
      <button onClick={() => setDismissed(true)} style={{
        ...
        background: "none", border: "none", color: "#ffffff40",
        fontSize: 18, cursor: "pointer", lineHeight: 1
      }}>×</button>
      <div style={{ fontSize: 20, marginBottom: 8 }}>🎯</div>
      <div style={{ fontSize: 15, fontWeight: "bold", color: "#f59e0b", marginBottom: 10 }}>
      ...
      <div style={{ fontSize: 13, color: "#ffffff70", lineHeight: 1.7, marginBottom: 12 }}>
        ...
        If a client or recruit comes out of it — amazing! But your <strong style={{ color: "#f0ede8" }}>#1 goal</strong> is to get reps
        in front of your trainer so you can sharpen your skills.
      </div>
      <div style={{
        background: "#ffffff0a",
...
function RepPhotoUpload({ photo, onUpdate }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 20 }}>
      ...
      {photo && (
        <button onClick={() => onUpdate(null)} style={{ background: "none", border: "none", color: "#f43f5e80", cursor: "pointer", fontSize: 11, marginTop: 6 }}>Remove photo</button>
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ background: "#ffffff10", borderRadius: 99, height: 12, overflow: "hidden", position: "relative" }}>
        <div style={{ width: `${p}%`, height: "100%", background: p >= 100 ? "#10b981" : `linear-gradient(90deg,${color},${color}cc)`, borderRadius: 99, transition: "width 0.6s ease", position: "relative" }}>
          {p >= 10 && <div style={{ position: "absolute", right: 6, top: 0, bottom: 0, display: "flex", alignItems: "center", fontSize: 9, color: "#0f0f11", fontWeight: "bold" }}>{p}%</div>}
        </div>
      </div>
...
function ProductionDashboard({ reps, trainers, admins, currentAdminId, isSuperAdmin, onUpdateRep }) {
  const [activeAdminFilter, setActiveAdminFilter] = useState(currentAdminId || "all");
  const [showRepInput, setShowRepInput] = useState(null);
  const [premiumInput, setPremiumInput] = useState("");

  // Filter reps based on admin
  const filteredReps = isSuperAdmin && activeAdminFilter === "all"
    ? reps
    : reps.filter(r => {
        const trainer = trainers.find(t => t.id === r.trainerId);
        return trainer?.adminId === (isSuperAdmin ? activeAdminFilter : currentAdminId) || r.adminId === (isSuperAdmin ? activeAdminFilter : currentAdminId);
      });
  ...
  // Top performers
  const topByPremium = [...filteredReps].filter(r => r.premiumSubmitted > 0).sort((a,b) => (b.premiumSubmitted||0) - (a.premiumSubmitted||0)).slice(0,5);

  // Per-trainer breakdown
  const trainerStats = trainers
    .filter(t => isSuperAdmin ? (activeAdminFilter === "all" || t.adminId === activeAdminFilter) : t.adminId === currentAdminId)
    .map(t => {
      const tReps = filteredReps.filter(r => r.trainerId === t.id);
      return {
        trainer: t,
        reps: tReps.length,
        premium: tReps.reduce((s,r) => s + (Number(r.premiumSubmitted)||0), 0),
        licensed: tReps.filter(r => r.isLicensed || r.examCompleted).length,
        appts: tReps.reduce((s,r) => s + (r.appointments||[]).filter(a=>a.completed||a.status==="completed").length, 0),
      };
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <div style={{ fontSize: 14, fontWeight: "bold", color: "#f59e0b", letterSpacing: "0.05em" }}>📊 Team Production Dashboard</div>
        {isSuperAdmin && (
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => setActiveAdminFilter("all")} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${activeAdminFilter === "all" ? "#f59e0b" : "#ffffff20"}`, background: activeAdminFilter === "all" ? "#f59e0b18" : "transparent", color: activeAdminFilter === "all" ? "#f59e0b" : "#ffffff50", cursor: "pointer", fontSize: 11, fontWeight: activeAdminFilter === "all" ? "bold" : "normal" }}>All</button>
            {admins.map(a => (
              <button key={a.id} onClick={() => setActiveAdminFilter(a.id)} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${activeAdminFilter === a.id ? a.color : "#ffffff20"}`, background: activeAdminFilter === a.id ? `${a.color}18` : "transparent", color: activeAdminFilter === a.id ? a.color : "#ffffff50", cursor: "pointer", fontSize: 11, fontWeight: activeAdminFilter === a.id ? "bold" : "normal" }}>{a.name}</button>
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ background: "#ffffff07", border: "1px solid #ffffff12", borderRadius: 14, padding: "20px 22px", marginBottom: 16 }}>
        ...
        <div style={{ borderTop: "1px solid #ffffff10", paddingTop: 14, marginTop: 4 }}>
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
            {[
              { label: "Total Appointments Done", value: apptsDone, color: "#f43f5e", emoji: "📅" },
            ...
            <div key={rep.id} style={{ background: "#ffffff05", border: "1px solid #ffffff10", borderRadius: 10, padding: "10px 14px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  {/* Premium input — licensed agents only */}
                  {(rep.isLicensed||rep.examCompleted) && showRepInput === rep.id ? (
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button onClick={() => { setShowRepInput(null); setPremiumInput(""); }}
                        style={{ background: "none", border: "1px solid #ffffff20", color: "#ffffff60", padding: "4px 10px", borderRadius: 6, cursor: "pointer", fontSize: 12 }}>✕</button>
                    </div>
                    ...
                    <div style={{ fontSize: 11, color: "#ffffff25", fontStyle: "italic" }}>License first to track premium</div>
                  )}
                </div>
              </div>
            </div>
...
export default function App() {
  ...
  // Take a snapshot of current month's production and reset
  const snapshotAndReset = () => {
    ...
    setMonthlyData(prev => ({ ...prev, [monthKey]: snapshot }));
    // Reset premium on all reps
    setReps(prev => prev.map(r => ({ ...r, premiumSubmitted: 0, isRecruited: false })));
    setShowNewMonthBanner(false);
  };

  // Check if new month on load
  useEffect(() => {
  ...
  const updateRepPhoto = (repId, photoData) => updateRep(repId, r => ({ ...r, photo: photoData }));
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"20px 16px" }}>
        ...
        {isAdmin && (
          <ProductionDashboard
            ...
            onUpdateRep={(repId, field, value) => { if (repId === "__snapshot__") { const snap = value; setMonthlyData(prev => ({ ...prev, [snap.month]: snap })); setReps(prev => prev.map(r => ({ ...r, premiumSubmitted: 0, isRecruited: false }))); } else updateRepProduction(repId, field, value); }}
